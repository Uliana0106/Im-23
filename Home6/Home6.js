...
AFRAME.registerComponent("run", {
    init: function () {
        ...
        this.perimeterText = document.querySelector("a-marker#A a-text"); // Added line
            ...
            for (let i = 0; i < activeMarkers.length - 2; i++) {
                for (let j = i + 1; j < activeMarkers.length - 1; j++) {
                    for (let k = j + 1; k < activeMarkers.length; k++) {
                        ...
                        // Set the value attribute of a-text with the calculated perimeter
                        this.perimeterText.setAttribute("value", `P = ${perimeter}`);
